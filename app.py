...
def get_video_info(filepath):
    ...
    try:
        ...
        ffprobe_path = ffmpeg_mgr.get_ffprobe_path() or 'ffprobe'
        cmd = [
...
def process_batch(job_id, files_info, shader, quality, mode, scale, custom_w, custom_h):
    ...
    for i, file_info in enumerate(files_info):
        ...
        width, height = calculate_resolution(
            vid_info, mode, scale, custom_w, custom_h)

        # Fix path separators for FFmpeg filter string (Windows/Linux compatibility)
        shader_path = os.path.join('shaders', shader).replace('\\', '/')

        # Ensure input/output paths are absolute to avoid any CWD confusion depending on how ffmpeg is spawned
        input_path_abs = os.path.abspath(input_path)
        output_path_abs = os.path.abspath(output_path)

        ffmpeg_path = ffmpeg_mgr.get_ffmpeg_path() or 'ffmpeg'
        cmd = [
            ffmpeg_path, '-y',
            '-i', input_path_abs,
        ...
        try:
                ...
                if line.startswith('frame=') and total_frames > 0:
                    ...
                    if match:
                        ...
                        jobs[job_id]['progress'] = int(
                            ((i + file_progress) / total_files) * 100)
            ...
            if process.returncode == 0:
                ...
                print(
                    f"FFmpeg failed for {input_filename}. Return code: {process.returncode}")
                print("FFmpeg Output/Error Log:")
                print("".join(output_log[-20:]))  # Print last 20 lines of log
                jobs[job_id]['error'] = f"FFmpeg failed: {process.returncode}"
...
def gpu_info():
    ...
    try:
        ...
        result = subprocess.run(
            [ffmpeg_path, '-hide_banner', '-init_hw_device', 'vulkan', '-f',
                'lavfi', '-i', 'nullsrc', '-t', '0.1', '-f', 'null', '-'],
...
def ffmpeg_status():
    ...
    status = ffmpeg_mgr.get_status()
    return jsonify(status)


@app.route('/ffmpeg-download', methods=['POST'])
def ffmpeg_download():
    ...
    def progress_callback(progress, status, message):
        pass  # Progress is tracked via /ffmpeg-status endpoint

    ffmpeg_mgr.download_async(progress_callback)
...
def start_processing():
    ...
    thread = threading.Thread(
        ...
        args=(job_id, files_info, shader, quality,
              mode, scale, custom_w, custom_h)
    )
    thread.daemon = True
    thread.start()
...
def job_status(job_id):
    ...
    if job_copy['status'] == 'processing':
        job_copy['elapsed'] = round(
            time.time() - job_copy.get('start_time', job_copy['created_at']), 1)
    elif job_copy.get('end_time'):
        job_copy['elapsed'] = round(
            job_copy['end_time'] - job_copy.get('start_time', job_copy['created_at']), 1)
    else:
        job_copy['elapsed'] = 0
...
def save_as_proxy(filename):
    ...
    try:
        ...
        save_path = active_window.create_file_dialog(
            webview.FileDialog.SAVE, save_filename=filename, file_types=file_types)
...
if __name__ == '__main__':
    ...
    # Initialize FFmpeg - download if not installed
    print("Checking FFmpeg installation...")
    if not ffmpeg_mgr.is_installed():
        print("FFmpeg not found. Starting download...")
        ...
        try:
            ffmpeg_mgr.download_and_install(progress_callback)
            print("✓ FFmpeg installed successfully!")
        except Exception as e:
            print(f"✗ FFmpeg download failed: {e}")
            print("  The app will try to use system FFmpeg if available.")
    else:
    ...
    webview.create_window('Anime4K-Offline-Upscaler',
                          f'http://127.0.0.1:{PORT}', width=1200, height=900)
    try:
        webview.start()
    except Exception as e:
